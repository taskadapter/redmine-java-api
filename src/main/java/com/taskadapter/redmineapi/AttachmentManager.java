...
public class AttachmentManager {
	 ...
	 *            the file to upload
	 * @param contentType
	 *            MIME type. depending on this parameter, the file will be recognized by the server as text or image or binary. see http://en.wikipedia.org/wiki/Internet_media_type for possible MIME
	 *            types. sample value: ContentType.TEXT_PLAIN.getMimeType()
	...
	public Attachment addAttachmentToIssue(Integer issueId, File attachmentFile, String contentType) throws RedmineException, IOException {
		final Attachment attach = uploadAttachment(attachmentFile, contentType);
	...
	public Attachment uploadAttachment(File file, String contentType) throws RedmineException, IOException {
		// final InputStream wrapper = new MarkedInputStream(content, "uploadStream");
		final String token;
		try {
	...
	public void downloadAttachmentContent(Attachment issueAttachment, OutputStream stream) throws RedmineException {
		transport.download(issueAttachment.getContentURL(), new CopyBytesHandler(stream));
	}
	 ...
	 */
	public InputStream downloadAttachmentContent(Attachment issueAttachment) throws RedmineException {
		...
		InputStream isreturn = new ByteArrayInputStream(baos.toByteArray());
		return isreturn;
	}
